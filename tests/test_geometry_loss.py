...
def _build_ideal_residue(res_name: str, base_z: float = 0.0) -> torch.Tensor:
    ...
    O  = C + torch.tensor([0.0, IDEAL["C_O"], 0.0])
    CB = CA + torch.tensor([0.0, 0.0, IDEAL["CA_CB"]])
    atoms = torch.zeros(15, 3)
    atoms[0] = N; atoms[1] = CA; atoms[2] = C; atoms[3] = O
    if res_name != "GLY":
        atoms[4] = CB
...
def test_bond_length_loss_near_zero_for_ideal_geometry():
    ...
    atoms = torch.stack([_build_ideal_residue(n, base_z=i * 10.0)
                         for i, n in enumerate(res_names)])  # [L, 15, 3]
