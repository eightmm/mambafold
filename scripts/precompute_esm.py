...
from mambafold.data.esm import ESMC_6B_MODEL_NAME, ESMEmbedder
...
def main():
    ...
    parser.add_argument("--esm_model", default=ESMC_6B_MODEL_NAME)
    ...
    parser.add_argument(
        "--limit", type=int, default=0, help="Debug: process only the first N input files."
    )
    ...
    if args.single_chain_fasta:
        ...
        for i, path in enumerate(files):
            ...
            if (i + 1) % 10000 == 0:
                print(
                    f"  scanned {i + 1}/{len(files)}, unique seqs so far: {len(seq_to_paths)}",
                    flush=True,
                )
    ...
    print(
        f"Phase 1 done: {n_unique} unique seqs → {n_total_outputs} outputs to write  "
        f"(scan_err={n_scan_err})",
        flush=True,
    )
